...
def suffle(dataset_x, dataset_y):
    indices = numpy.arange(dataset_x.shape[0])
    numpy.random.shuffle(indices)

    return dataset_x[indices], dataset_y[indices]


def trainCorrNet2(dataset_x, dataset_y, test_x, test_y, batch_size = 20, training_epochs=40,
    ...
    model.init(numpy_rng=rng, theano_rng=theano_rng, l_rate=l_rate, optimization=optimization, tied=tied,
               n_visible_left=n_visible_left, n_visible_right=n_visible_right, n_hidden=n_hidden, lamda=lamda,
               W_left=W_left, W_right=W_right, b=b, W_left_prime=W_left_prime, W_right_prime=W_right_prime,
               b_prime_left=b_prime_left, input_left=x_left, input_right=x_right, hidden_activation=hidden_activation,
               output_activation=output_activation, loss_fn =loss_fn)

    start_time = time.clock()

    train_set_x_left = theano.shared(numpy.asarray(numpy.zeros((dataset_x.shape[0], dataset_x.shape[1])), dtype=theano.config.floatX),
                                     borrow=True)
    train_set_x_right = theano.shared(numpy.asarray(numpy.zeros((dataset_x.shape[0], dataset_x.shape[1])), dtype=theano.config.floatX),
                                      borrow=True)
    ...
    for epoch in xrange(training_epochs):
        ...
        c = []
        train_x, train_y = suffle(dataset_x, dataset_y)
        train_set_x_left.set_value(train_x)
        train_set_x_right.set_value(train_y)
        for batch_index in range(0, int(dataset_x.shape[0])/batch_size):
            c.append(mtrain_common(batch_index))

    view_x = model.proj_from_left(test_x)
    view_y = model.proj_from_right(test_y)
